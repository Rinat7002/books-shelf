...
# @login_required
def skills():
    return render_template('books.html', title='Книги', navmenu=navmenu)

@app.route('/articles')
def articles():
    return render_template('articles.html', title='Статьи', navmenu=navmenu)
...
def account():
    ...
    print(response)
    return render_template('account.html', title='Профиль', navmenu=navmenu, all_reqs=response)
